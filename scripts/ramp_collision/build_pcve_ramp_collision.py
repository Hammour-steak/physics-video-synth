"""Build the ramp_collision PCVE suite.
...
EDIT_CASES: tuple[EditCase, ...] = (
    EditCase(
        ...
        edit_summary=(
            ...
            "extra rolling resistance eats it on the tabletop: it stops 21 cm "
            "short of blue and there is no collision."
        ),
    ),
    EditCase(
        ...
        source_case_id=SOURCE_CASE_ID,
        seed=2102,
        dsl="DELETE blue_marble",
        edit_summary=(
            "Blue marble removed. Red rolls down through the empty spot; "
            "yellow is off to the side and is never touched."
        ),
    ),
    EditCase(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build the ramp_collision PCVE suite (1 source + N edits)."
    )
    parser.add_argument(
        "--out-root",
        type=Path,
...
def main() -> None:
    ...
    manifest: dict[str, Any] = {
        ...
        "description": (
            "One source ramp-collision video plus N edited variants. Each edit "
    ...
    if args.skip_existing and expected_source_video.exists():
        ...
        render_case(args, case_dir=source_dir, seed=2001, overrides_path=None)
