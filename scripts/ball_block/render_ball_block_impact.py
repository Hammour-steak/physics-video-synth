...
def jittered_color(
    ...
    """Kept for call-site parity; the scene is deterministic so nothing moves."""
    del rng, amount
    return color
...
def create_side_impact_motion(
    ...
    return MotionSetup(
        ball_initial_location=(
            SIDE_IMPACT_BALL_INITIAL_XY[0],
            SIDE_IMPACT_BALL_INITIAL_XY[1],
            BALL_RADIUS + 0.001,
        ),
...
def create_drop_onto_block_motion(
    ...
    drop_x_velocity = (
        0.18 if args.drop_x_velocity is None else float(args.drop_x_velocity)
    )
    drop_y_velocity = (
        0.0 if args.drop_y_velocity is None else float(args.drop_y_velocity)
    )
    ball_initial_location = (
        block_location[0] - 0.16,
        block_location[1],
        block_top_z + BALL_RADIUS + 1.32,
    )
    ball_initial_velocity = (
        drop_x_velocity,
        drop_y_velocity,
...
# Camera constants. Every other scene in the benchmark fixes its camera this way;
# this one used to draw position, aim and focal length from `rng`, and since each
# PCVE case renders with its own --seed (5101, 5102, ... against the source's
# 5001) the viewpoint and lens moved between an edited case and its own source.
# A pixel-space comparison then measures the viewpoint change, not the edit.
SIDE_IMPACT_CAMERA_LENS_MM = 46.0
SIDE_IMPACT_CAMERA_FSTOP = 6.2
DROP_CAMERA_OFFSET = (0.0, -0.22, 0.42)
DROP_CAMERA_TARGET = (WOOD_BLOCK_LOCATION[0] - 0.05, WOOD_BLOCK_LOCATION[1], 0.88)
DROP_CAMERA_LENS_MM = 41.0
DROP_CAMERA_FSTOP = 7.0

def create_side_impact_camera(rng: random.Random) -> dict[str, object]:
    del rng  # the camera is fixed; kept in the signature for call-site parity
    base_location = list(CAMERA_LOCATION)
    target = list(CAMERA_TARGET)
    return {
        "base_location": base_location,
        "target": target,
        "lens_mm": SIDE_IMPACT_CAMERA_LENS_MM,
        "sensor_width_mm": 32.0,
        "focus_distance": math.dist(base_location, target),
        "aperture_fstop": SIDE_IMPACT_CAMERA_FSTOP,
    }
...
def create_drop_onto_block_camera(
    ...
    # block_location is itself jittered per case, so the aim point comes from the
    # nominal block position instead -- that keeps the camera identical across
    # cases even when the block is not pinned through scenario overrides.
    del block_location, rng  # camera is fixed
    base_location = [
        CAMERA_LOCATION[0] + DROP_CAMERA_OFFSET[0],
        CAMERA_LOCATION[1] + DROP_CAMERA_OFFSET[1],
        CAMERA_LOCATION[2] + DROP_CAMERA_OFFSET[2],
    ]
    target = list(DROP_CAMERA_TARGET)
    return {
        "base_location": base_location,
        "target": target,
        "lens_mm": DROP_CAMERA_LENS_MM,
        "sensor_width_mm": 32.0,
        "focus_distance": math.dist(base_location, target),
        "aperture_fstop": DROP_CAMERA_FSTOP,
    }
...
def create_scenario(args: argparse.Namespace) -> dict[str, object]:
    ...
    block_location = (
        WOOD_BLOCK_LOCATION[0],
        WOOD_BLOCK_LOCATION[1],
    ...
    # Eight scuffs spread over the ball on a fixed golden-angle spiral, so the
    # marks look scattered but land in the same place on every render.
    extra_scuffs = []
    golden_angle = math.pi * (3.0 - math.sqrt(5.0))
    for i in range(8):
        z = -0.25 + (0.92 - (-0.25)) * (i / 7.0)
    ...
    scenario = {
        ...
        "render": {
            # Perturbations around neutral collapse to 0; parameters that were
            # drawn from a range with no base value take that range's midpoint.
            "exposure": 0.0,
            "gamma": 1.0,
            "motion_blur_shutter": 0.30,
            "postprocess_noise_strength": 3,
            "postprocess_contrast": 1.012,
            "postprocess_saturation": 1.012,
            "postprocess_gamma": 0.998,
            "lens_correction_k1": -0.014,
            "lens_correction_k2": 0.0055,
        ...
        "materials": {
            ...
            "floor_texture_scale": 48.0,
            "block_texture_scale": 0.95,
            "ball_texture_scale": 3.8,
...
def add_background_clutter(scenario: dict[str, object]) -> None:
    # Fixed placement: this ran off `seed`, so the rail angle and the furniture
    # legs shifted between a case and its own source even though both are meant
    # to differ only by the edited property.
    del scenario
    ...
    add_box("low background wooden rail", (2.62, 2.64, 0.78), (1.85, 0.18, 0.16), dark_wood, rotation_z=0.0, bevel_width=0.012)
